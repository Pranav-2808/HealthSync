...
import { Users, Activity, AlertTriangle, ShieldAlert, X } from "lucide-react";
import { motion, AnimatePresence } from "motion/react";

const Dashboard: React.FC = () => {
  const { members, dismissedIds, dismissAlert } = useApp();

  const activeCriticalMembers = members.filter(m => {
    const hr = m.healthData?.hr || 0;
    const spo2 = m.healthData?.spo2 || 100;
    const isCritical = hr > 105 || hr < 50 || spo2 < 93;
    return isCritical && !dismissedIds.has(m.id);
  });
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <header className="mb-10">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-display font-bold text-slate-900 tracking-tight">Trainer Dashboard</h1>
            <p className="text-slate-500 mt-1">Real-time health monitoring for all gym members</p>
          </div>
          ...
          <div className="space-y-4 mb-10">
            {activeCriticalMembers.map(member => (
              <motion.div 
                ...
                className="bg-white border-2 border-rose-500 rounded-[2.5rem] shadow-2xl shadow-rose-100 overflow-hidden"
              >
                  ...
                  <div className="p-8 flex flex-1 items-center gap-8">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        ...
                        <AlertStat label="Emergency Contact" value={member.emergencyContact.phone} />
                      </div>
                    </div>
                  </div>
                  
                  <div className="p-8 lg:w-[320px] bg-slate-50 flex flex-col justify-center gap-4 shrink-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        {members.length === 0 && (
          <div className="col-span-full py-20 text-center glass rounded-3xl">
            ...
            <p className="text-slate-500">Register new members to start monitoring</p>
          </div>
        )}
      </div>
    </div>
...
const StatSmall: React.FC<{ icon: React.ReactNode, label: string, value: number, isAlert?: boolean }> = ({ icon, label, value, isAlert }) => (
  <div className="glass px-5 py-3 rounded-2xl flex items-center gap-4">
    ...
    <div>
      ...
      <p className={`text-xl font-display font-bold ${isAlert ? "text-rose-600" : "text-slate-900"}`}>{value}</p>
    </div>
  </div>
);
